...
async function parsePDF(pdf, options = {}) {
  ...
  for (let pageNum = 1; pageNum <= pdf.numPages; pageNum++) {
    ...
    if (shouldProcessCash) {
      ...
      // FIX: Search for headers in the *full* item list for the page, not a clipped version.
      // The premature clipping based on cashStartMarker was removing the headers themselves.
      let cashHeaders = findCashHeaders(items);

      if (cashHeaders) {
        cashColumnBoundaries = calculateCashColumnBoundaries(cashHeaders);
        debugLog('Found/updated Cash headers/boundaries:', cashColumnBoundaries);
      } else {
        debugLog('No new cash headers found on this page, will reuse previous if available.');
      }

      if (cashColumnBoundaries) {
        // Pass the full `items` list. extractTransactionsFromPage will correctly
        // filter for items below the header line itself.
        const pageCashTransactions = extractTransactionsFromPage(items, cashColumnBoundaries, 'cash');
    ...
    if (cashEndMarker) {
      debugLog('Cash section ended.');
      isParsingCash = false;
      cashColumnBoundaries = null; // Reset boundaries after section ends
    ...
    if (shouldProcessInterest) {
      ...
      // Apply the same logic fix for interest headers
      let interestHeaders = findInterestHeaders(items);

      if (interestHeaders) {
        interestColumnBoundaries = calculateInterestColumnBoundaries(interestHeaders);
        debugLog('Found/updated Interest headers/boundaries:', interestColumnBoundaries);
      } else {
        debugLog('No new interest headers found, will reuse previous if available.');
      }

      if (interestColumnBoundaries) {
        const pageInterestTransactions = extractTransactionsFromPage(items, interestColumnBoundaries, 'interest');
    ...
    if (interestEndMarker) {
      debugLog('Interest section ended.');
      isParsingInterest = false;
      interestColumnBoundaries = null; // Reset boundaries
  ...
  // Trigger log download (DISABLED BY DEFAULT)
  // downloadLogFile();
