  ...
  for (let pageNum = 1; pageNum <= pdf.numPages; pageNum++) {
    ...
    if (shouldProcessCash) {
      ...
      if (cashColumnBoundaries) {
        ...
        // también registrar en el debug UI si existe
        try {
          const dbg = document.getElementById('debug');
          if (dbg) {
            const note = document.createElement('div');
            note.innerHTML = `<strong>Page ${pageNum} - Cash headers:</strong> ${JSON.stringify(cashColumnBoundaries)}`;
            dbg.appendChild(note);
          }
        } catch (e) { /* noop */ }
    ...
    if (shouldProcessInterest) {
      ...
      if (interestColumnBoundaries) {
        ...
        try {
          ...
          if (dbg) {
            const note = document.createElement('div');
            note.innerHTML = `<strong>Page ${pageNum} - Interest headers:</strong> ${JSON.stringify(interestColumnBoundaries)}`;
            dbg.appendChild(note);
          }
        } catch (e) { /* noop */ }
