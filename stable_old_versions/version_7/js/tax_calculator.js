  ...
  preparedTxs.forEach(tx => {
    ...
    if ((type === 'Operar' || type === 'Trade' || type === 'Handel') && tx.qty > 0) {
      ...
      // DETECTAR VENTA (Entrada de dinero > 0)
        ...
        const proceeds = tx.inc; // Neto recibido
        const commission = 1.0; // Comisión fija estimada

        // Resultado Bruto = Venta Neta - Coste Base
        const grossResult = proceeds - totalCostBasis;
        ...
        realizedPnL.push({
          ...
          precio_venta_unitario: round3(proceeds / tx.qty),
          coste_base: round3(totalCostBasis),
          importe_venta_neto: round3(proceeds),
...
function buildNestedFifoStructure(flatList, roundFn) {
  ...
  flatList.forEach(item => {
    ...
    entry.bruto += item.importe_venta_neto;
