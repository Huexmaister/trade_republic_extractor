...
function renderTradingComponent(tradingData, tradingTransactions) {
  ...
  fifoButton.onclick = () => {
    if (fifoData) {
      ...
      link.href = url;
      link.download = 'fifo_report.json';
  ...
  incomeButton.onclick = () => {
    if (fifoData) {
      ...
      const link = document.createElement('a');
      link.href = url;
      link.download = 'income_report.json';
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
    } else {
...
function renderComponent(title, rows, prefix, options = {}) {
  ...
  mkBtn('Exportar Extracto (JSON)', () => jsonDL(rows, name));
...
function renderSupportComponent({ cashCount = 0, mmfCount = 0, tradingCount = 0, failedChecks = 0 } = {}) {
  ...
  info.innerHTML = `
    ...
    <p class="mt-2">Haz un fork del proyecto, adáptalo a tus propios flujos de trabajo o amplía los formatos de exportación según sea necesario.</p>
  `;
